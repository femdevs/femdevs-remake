...
const cd = {
	...
	encoding: 'base64url' as BufferEncoding,
};
...
Object.assign(
	...
	Object.fromEntries(
		['key', 'iv']
			.map(k => [k, 
				Buffer.from(process.env[`C_${k}`.toUpperCase()] as string, cd.encoding).toString(cd.encoding),
			])
	)
);
...
class Cryptography {
	static encrypt(inputData) {
		...
		const encdata = crypto.createCipheriv(
			cd.crypt!.name as crypto.CipherGCMTypes,
