...
class Index {
    static Crypt = class {
        ...
        static Auto = class {
            static encrypt(inputData: string) {
            ...
            static verify(inputData: string) {
            ...
            static decrypt(inputData:string) {
                const data = JSON.parse(Buffer.from(inputData, Index.Crypt.cd.encoding).toString('utf-8'));
                Object.keys(new Index.Crypt.CryptographyData()).forEach(key => assert(Object.hasOwn(data, key), `Missing ${key} value`));
                const obj = new Index.Crypt.CryptographyData();
                for (const key of Object.keys(obj)) {
        ...
        static Manual = class {
            static encrypt = (id: string, key: string, iv: string) => {
            ...
            static verify = (token: string, key: string, iv: string) => {
            ...
            static decrypt = (token: string, key: string, iv: string) => {
        ...
        static completeHash = (data: any, fa = 'id-rsassa-pkcs1-v1_5-with-sha3-512') => {
            const hash = crypto.createHash(fa).update(crypto.getHashes().reduce((data: string, algo: string) => crypto.createHash(algo).update(data).digest(), data)).digest(Index.Crypt.cd.encoding);
