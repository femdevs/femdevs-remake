...
export async function GET(request) {
    ...
    const loc = request.headers.get('x-coords');
    if (!loc) return sendError(6);
    const results = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?key=${process.env.GMAPS_API_KEY}&latlng=${loc}`);
