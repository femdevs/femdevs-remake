...
import Auth from '#/src/crypto';
import htmlProcessor from 'node-html-parser';
...
class MailReqProcessor {
    ...
    static async Send(req) {
        const resp = new NextResponse(null, { status: 201 });
        const auth = this.getAuth(req.headers.get('x-user'));
        const from = this.from(req.headers.get('x-from'));
        const body = await this.parseBody(req.body);
        if (!auth || !from) return sendError(8);
        if (!body) return sendError(10);
...
export async function POST(request) {
    return MailReqProcessor.Send(request);
}
